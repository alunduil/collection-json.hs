...
unlabelled issue, or a PR for something not yet filed, is equally
welcome.
...
Run `cabal update` on a first checkout and after any gap between
contributions. A stale Hackage index surfaces as a dependency
resolution failure that reads like a broken build.
...
Commits inside the PR are collapsed on merge, so their messages only
need to hold up under commit-by-commit review.
...
issues too; tag them [`question`][question].
