...
Addresses ('cHref', 'lHref', 'iHref', and 'qHref') decode as URI
references, so relative forms are accepted. A relative address resolves
against the address the document was retrieved from, as described in
...
{- Parses a URI reference instead of delegating to aeson's @FromJSON URI@,
which accepts absolute URIs only. The @href@ fields hold an unconstrained
'URI', and @ToJSON URI@ encodes relative references, so an absolute-only
decoder would refuse input this library itself emits. Enforcing absolute
addresses needs a type that cannot hold a relative one.
-}
